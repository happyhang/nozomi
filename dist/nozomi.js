...
if (!fs.existsSync(outputPath)) {
  throw new Error(source.red('Output path does not exist!'));
}

const outputStats = fs.lstatSync(outputPath);
if (!outputStats.isDirectory()) {
  throw new Error(source.red('Output path is not a directory. You must specify a directory for output.'));
}

console.log(source.yellowBright('Nozomi, starts to sing!'));

// Stream the sound manifest file, look for entries prefixed with 'b'.
let lineRaw;
let isReadDone = false;
const bgmFileList = [];
const liner = new readlines(soundManifest);
...
console.log(source.cyan(`Found ${bgmFileList.length} BGMs.`));
...
console.log(source.green('All done. Enjoy!'));
